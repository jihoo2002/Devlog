...
개발자가 좋은 서비스를 만들었다면,<br>
그 서비스에 누구나 접속하고 이용할 수 있도록 공개하는 과정이 바로 배포이다.

따라서 어떤 서비스를 완성했다면, 다음 단계는 배포가 된다.
...
많은 클라우드 서비스가 존재하지만, 대표적으로 `AWS`, `Microsoft Azure`,<br>
그리고 `GCP`가 클라우드 시장에서 큰 점유율을 차지하고 있다.
...
> AWS는 전 세계적으로 다양한 Region을 보유하고 있다.<br>

`EC2`가 하나의 컴퓨터를 뜻하고 이러한 컴퓨터들이 전세계적으로 다양하게 분포되어 있는데,<br>
`AWS`에서는 이 `컴퓨터들이 위치한 지역 단위`를 리전이라고 부른다.

---

#### 👉 그럼 리전은 어떤 기준으로 선택할까?
애플리케이션을 사용하는 사용자들의 위치와 지리적으로 가까운 리전을 선택하는 것이 유리하다.

> 우리는 주로 한국 유저들이 사용하는 서비스를 만들거라 가정한다면<br>
> 리전을 `아시아 태평양(서울)`로 선택하면 된다.

---

#### 👉 리전 숨바꼭질 주의..
서울 리전에 `EC2`를 생성해놓고, 다시 내 `EC2`를 확인하기 위해 `aws` 홈페이지에 접속했다.<br>
그런데 생성한 `EC2`가 보이지 않는다면?

대부분 리전이 다른 곳으로 `선택`되어 있어서다!<br>
항상 리전 선택이 올바르게 선택되었는 지 확인하는 습관을 들이자.
...
### 자바 설치하기
```bash
sudo apt install openjdk-17-jdk -y # 본인 프로젝트에서 사용하는 Java 버전에 맞춰 설치
...
`HTTP`는 요청과 응답이 암호화되지 않은 상태로 전달되기 때문에,<br>
중간에서 누군가 데이터를 훔쳐보거나 변조할 위험이 있다.

반면에 `HTTPS`는 `TLS(SSL)` 암호화를 사용하여 데이터를 안전하게 전송하고,<br>
...
각자 원하는 도메인으로 설정 후 `add domain` 클릭

>제가 사용한 도메인은 이미 등록되어 있으니, 여러분들은 원하는 다른 이름으로 도메인을 생성하시면 됩니다!
...
```bash
# 꼭 http://<본인의 EC2 퍼블릭 IP>:8080 을 입력해주세요!
location / {
...
### 더 알아볼 부분
인스턴스가 중지되고 다시 실행할 때마다 본인의 IP가 바뀝니다. 어떻게 해결해야 할까요?
- 힌트 : 탄력적 IP 할당 방법

매번 새로운 기능을 추가할 때마다 이렇게 복잡한 배포 과정을 반복해야 할까요?
